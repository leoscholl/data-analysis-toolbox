function ConvertLogMats( DataDir, AnimalID, WhichUnits, WhichFiles )
...
if nargin < 3
    WhichUnits = FindUnits(DataDir, AnimalID);
end
if isnumeric(WhichUnits)
    WhichUnits = FindUnits(DataDir, AnimalID, WhichUnits);
end
...
for i = 1:size(WhichUnits,1)
    ...
    Unit = deblank(WhichUnits(i,:));
    DataPath = fullfile(DataDir,AnimalID,Unit,filesep);
    
    Files = FindFiles(DataDir, AnimalID, Unit, '*].mat', WhichFiles);

    if isempty(Files)
        warning(['No log mat files found for ', Unit]);
    end
    ...
    for fi = 1:size(Files,1)
        ...
        if exist('Params','var') && isfield(Params,'Data')
            % Export them and we're done
            if exist([DataPath,FileName,'-export.mat'],'file')
                save([DataPath,FileName,'-export.mat'],'Params','-append');
            else
                save([DataPath,FileName,'-export.mat'],'Params');
            end
            disp(['exported parameters to ', FileName]);
            continue;
        else
        ...
        load(fullfile(DataPath,FileName), Vars{:});
        StimType = regexp(FileName, '\[(\w+)\]','tokens');
        StimType = char(StimType{1});
        Params.StimType = StimType;
        Params.ExpNo = regexp(FileName, '#(\d+)\[','tokens');
        Params.ExpNo = str2num(char(Params.ExpNo{1}));
        Params.AnimalID = AnimalID;
        Params.Unit = char(Unit);
        ...
        FileName = [FileName,'-export.mat'];
        FilePath = fullfile(DataPath, FileName);
        if ~exist(FilePath, 'file')
            save(FilePath, 'Params');
        else
            save(FilePath, 'Params', '-append');
        end
        disp(['converted parameters to ', FileName]);
    end % files loop
end % units loop
