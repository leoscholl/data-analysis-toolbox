function plotAllResults(figuresPath, fileName, Results,...
    ...
    plotMaps, summaryFig, plotLFPs, showFigures, isparallel)
...
if nargin < 12 || isempty(isparallel)
    isparallel = true;
end
if nargin < 11 || isempty(showFigures)
    showFigures = 0;
end
if nargin < 10 || isempty(plotLFPs)
    plotLFPs = 0;
end
...
if isparallel
    parfor i = 1:length(whichElectrodes)
        ...
        plotSingle(SpikeData, Statistics, LfpData, Params, i, elecNo, ...
            figuresPath, fileName, plotTCs, plotBars, plotRasters, plotMaps, ...
            summaryFig, plotLFPs, showFigures);
...
function plotSingle(SpikeData, Statistics, LfpData, Params, i, elecNo, ...
    figuresPath, fileName, plotTCs, plotBars, plotRasters, plotMaps, ...
    summaryFig, plotLFPs, showFigures)
...
% LFP
if plotLFPs && ~isempty(LfpData) && ~isempty(LfpData.Data)
    figBaseName = [fileName,'_','El',num2str(LfpData.electrodeid)];
    lfpFig = plotLFP(LfpData, Params, showFigures);
    saveas(lfpFig, fullfile(figuresPath,elecDir,[figBaseName,'_lfp.png']));
end
...
for j = 1:length(cells)
    ...
    if plotTCs
        ...
        if ~(isempty(findstr(fileName,'R')) || ...
            ...
            velStatsUnit = StatsUnit;
            velStatsUnit.conditionNo = velConditionNo;
            
            % Plot a figure
            velCurveFig = plotTCurve(velStatsUnit, ...
        ...
        if summaryFig == 1 && Params.nElectrodes>1
            ...
            delete(sub);
            H = copyobj([findobj(tCurveFig, 'type', 'axes'), ...
                findobj(tCurveFig, 'type', 'legend')],gcf);
    ...
    if plotRasters == 1 % one raster per condition
        ...
        [hRaster, hPSTH] = plotRastergrams( SpikeDataUnit, StatsUnit, ...
            Params, cellColors(j,:), showFigures, elecNo, u);
    ...
    if plotMaps
        ...
        if strfind(Params.stimType, 'BackProject')
            ...
            binSize = min(Params.ConditionTable.binSize);
