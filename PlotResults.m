function PlotResultsLeoBeta(ResultsPath, FileName, StimType, ...
...
Params.NoOfElectrodes = length(unique(SpikeTimesAll(:,1)));
...
Params.Data.StimTime = StimTimes(1:nStims);
Params.Data.StimDiffTime = [StimTimes(2:nStims); NaN] - StimTimes(1:nStims);
...
for el = 1:Params.NoOfElectrodes
    ...
    SpikeTimes = SpikeTimesAll(SpikeTimesAll(:,1) == el,2:end);
    
    ElectrodeName = ElectrodeNames{el};
...
%                 '-png', '-m2', '-p0.05', hMap);
    ...
    % Save everything
    Results(el).ElectrodeName = ElectrodeName;
